...
def readin_data_train(datadir,readindata,batch_size):
    ...
    prolong_length = 32 # originally was 8
...
class ResnetEncoder(nn.Module):
    ...
    def __init__(self, block, decodeblock, num_blocks, num_outputs):
        ...
        self.layer1 = self._make_layer(block, self.inplanes, 128*2, num_blocks[0])
        self.layer2 = self._make_layer(block, 128*2, 128*2, num_blocks[1]) #, stride=2)
        self.conv2 = nn.Conv1d(128*2, 64*2, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm1d(64*2)
        self.maxpool2 = nn.MaxPool1d(kernel_size=2, stride=1, padding=0)
        self.conv3 = nn.Conv1d(64*2, 128*2, kernel_size=2, stride=2, padding=0, bias=False)

        self.decode_t_conv1 = nn.ConvTranspose1d(128*2, 64*2, kernel_size=4, stride=4, padding = 1, bias=False)
        self.decode_bn1 = nn.BatchNorm1d(64*2)
        self.decode_layer1 = self._make_layer(decodeblock, 64*2, 64*2, num_blocks[0])
        self.decode_layer2 = self._make_layer(decodeblock, 64*2, 64*2, num_blocks[1])
        #self.decode_t_conv2 = nn.ConvTranspose1d(64, 64, kernel_size=4, stride=4, padding = 2, bias=False)
        self.decode_t_conv2 = nn.ConvTranspose1d(64*2, 64*2, kernel_size=16, stride=3, padding = 0, output_padding =0, bias=False)
        #self.decode_t_conv2 = nn.ConvTranspose1d(64*2, 64*2, kernel_size=13, stride=3, padding = 0, output_padding =0, bias=False)

        self.decode_bn2 = nn.BatchNorm1d(64*2)
        #self.decode_t_conv3 = nn.ConvTranspose1d(64, 1, kernel_size=3, stride=3, padding = 2, output_padding=1, bias=False)
        self.decode_t_conv3 = nn.ConvTranspose1d(64*2,1,kernel_size=15,stride=3,padding=0,output_padding=0, bias=False)
        #self.decode_t_conv3 = nn.ConvTranspose1d(64*2,1,kernel_size=12,stride=3,padding=0,output_padding=0, bias=False)
        
        self.predictor_conv1 = nn.Conv1d(128*2, 128*2, kernel_size=3, stride=1, padding=1, bias=False)
        self.predictor_bn1 = nn.BatchNorm1d(128*2)

        self.predictor_layer1 = self._make_layer(block, 128*2, 128*2, num_blocks[0])
        self.predictor_layer2 = self._make_layer(block, 128*2, 128*2, num_blocks[1])

        self.predictor_conv2 = nn.Conv1d(128*2, 128*2, kernel_size=3, stride=1, padding=1, bias=False)
        self.predictor_bn2 = nn.BatchNorm1d(128*2)
        self.predictor_conv3 = nn.Conv1d(128*2, 128*2, kernel_size=3, stride=1, padding=1, bias=False)
        #self.predictor_fc = nn.Linear(in_features=896, out_features=2)
        self.predictor_fc = nn.Linear(in_features=9472, out_features=2) #4608 9216
